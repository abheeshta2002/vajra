...
| 19 | Standard utilities (CLI-OS parity — usability, not the thesis) | ✅ Done — `ls cat cp mv rm grep edit ps`, each a separately loaded program that starts with no authority; the shell delegates just what each command needs, under a scoped `CAP_USER_DATA` and a per-actor `CAP_INTROSPECT` |
| 20 | Package installation, using quarantine for real | ✅ Done (local catalog) — `pkg list` / `pkg install <name>` in the shell: stage untrusted, inspect in a sandbox, then promote or permanently reject, under a dedicated narrow `CAP_INSTALL_PACKAGE`. Remote sources wait on Phase 29 and payload fragmentation |
