...
foreach ($u in $UtilNames) {
    ...
    if ($total -gt 12288) { Write-Host "FATAL: util_$u.bin ($total bytes) does not fit in one 12288-byte storage object" -ForegroundColor Red; exit 1 }
    if ($total -gt 7500) { Write-Host "WARNING: util_$u.bin is $total bytes -- CI's compiler emits noticeably more code than this one; keep large programs well under the 12288-byte object limit" -ForegroundColor Yellow }
